...
function CalendarExerciseCard({ exercise, completed, onToggle }: ExerciseCardProps) {
  return (
    <View
      className="bg-white rounded-2xl p-4 mb-3"
      style={{
        borderWidth: 1,
        borderColor: completed ? '#e5f7f5' : '#F3F4F6',
        elevation: 2,
        shadowColor: '#000',
    ...
    >
      ...
      <View style={{ gap: 8 }}>
        <TouchableOpacity
          ...
          style={{ height: 42, borderWidth: 1.5, borderColor: '#00A896' }}
          activeOpacity={0.7}
        >
          <View className="flex-row items-center" style={{ gap: 6 }}>
            <Ionicons name="play-circle-outline" size={16} color="#00A896" />
            <Text className="text-sm font-semibold" style={{ color: '#00A896' }}>
              Video Demo
            </Text>
          </View>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          style={{
            height: 42,
            backgroundColor: completed ? '#e5f7f5' : '#F9FAFB',
            borderWidth: 1,
            borderColor: completed ? '#00A896' : '#E5E7EB',
          }}
          onPress={onToggle}
          activeOpacity={0.75}
        >
          <View className="flex-row items-center" style={{ gap: 6 }}>
            <Ionicons
              ...
              size={16}
              color={completed ? '#00A896' : '#9CA3AF'}
            />
            <Text
              className="text-sm font-medium"
              style={{ color: completed ? '#00A896' : '#6B7280' }}
            >
      ...
      <ScrollView showsVerticalScrollIndicator={false} stickyHeaderIndices={[1]}>
        ...
        <View className="px-5 pt-4 pb-4">
          ...
          <View
            ...
            style={{ backgroundColor: '#e5f7f5', borderWidth: 1, borderColor: '#b2ece6' }}
          >
            <View
                ...
                >
                  ...
                  {hasExercises && !active && (
                    <View
                      ...
                      style={{ backgroundColor: dayCompleted ? '#16A34A' : '#00A896' }}
                    />
                  )}
                </TouchableOpacity>
              );
          ...
          {dayExercises.length === 0 ? (
            <View className="items-center py-16">
              ...
              <Text className="text-gray-400 text-base mt-3 font-medium">
                Sin ejercicios para {selectedDay}
              </Text>
              <Text className="text-gray-300 text-sm mt-1">¡Día de descanso!</Text>
            </View>
          ) : (
            dayExercises.map((ex) => (
