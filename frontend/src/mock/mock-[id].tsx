...
import {
  ...
  ViewStyle,
  TextStyle,
  Modal,      // <-- NUEVO: Para el pop-up de edición
  TextInput,  // <-- NUEVO: Para los inputs de edición
} from 'react-native';
...
const MOCK_PATIENTS: Record<string, Patient> = {
  '1': { id: '1', name: 'Carlos Rodríguez', diagnosis: 'Lesión de rodilla izquierda', period: '11/5/2026 al 31/5/2026', adherence: 85, status: 'Activo', medicalNotes: 'El paciente refiere molestia leve en la cara lateral externa al realizar flexión profunda. Monitorear rangos anulares en sentadillas.', durationWeeks: 3 },
  '2': { id: '2', name: 'María González', diagnosis: 'Rehabilitación de tobillo', period: '10/5/2026 al 7/6/2026', adherence: 92, status: 'Activo', medicalNotes: 'Sin observaciones adicionales.', durationWeeks: 4 },
  '3': { id: '3', name: 'Juan Pérez', diagnosis: 'Fortalecimiento de cuádriceps', period: '1/4/2026 al 27/4/2026', adherence: 100, status: 'Finalizado', medicalNotes: 'Tratamiento completado exitosamente.', durationWeeks: 4 },
};
...
// <-- NUEVO: Agregamos onEdit y onDelete a las propiedades (props) de la tarjeta
...
export default function PatientDetail() {
  ...
  const [weeks, setWeeks] = useState(patient?.durationWeeks || 3);
  const [routine, setRoutine] = useState<Record<Day, RoutineExercise[]>>(MOCK_ROUTINE);
  ...
  const [editName, setEditName] = useState(''); // Solo para mostrarlo en el título
  ...
  useEffect(() => {
    if (newExerciseName && newExerciseDay) {
      setRoutine(prev => {
        const dayRoutine = prev[newExerciseDay as Day] || [];
        if (dayRoutine.some(ex => ex.name === newExerciseName)) return prev;

        const newEx: RoutineExercise = {
          id: `temp-${Date.now()}`,
          name: newExerciseName,
          series: Number(newExerciseSets) || 3,
          reps: Number(newExerciseReps) || 15,
          muscle: newExerciseMuscle || 'General',
          completed: false,
          angle: newExerciseAngle || '-' 
        };

        return {
          ...prev,
          [newExerciseDay as Day]: [...dayRoutine, newEx]
        };
      });
  ...
  // --- LÓGICA DE BORRADO ---
  const handleDeleteExercise = (day: Day, exId: string) => {
    // Usamos el confirm estándar de la web para la validación rápida
    const confirmed = window.confirm("¿Estás seguro que querés borrar este ejercicio?");
    if (confirmed) {
      setRoutine(prev => ({
        ...prev,
        [day]: prev[day].filter(ex => ex.id !== exId) // Filtramos y quitamos el ejercicio borrado
      }));
    }
  };
  ...
  const handleOpenEdit = (day: Day, ex: RoutineExercise) => {
    ...
    // Truquito para desarmar "60° - 90°" y ponerlo en los inputs correspondientes
    let min = '';
    let max = '';
    if (ex.angle && ex.angle !== '-') {
      const angleString = ex.angle.replace(/°/g, ''); // Quitamos los símbolos de grados
      const parts = angleString.split('-'); // Cortamos por el guion
      if (parts.length === 2) {
        min = parts[0].trim();
        max = parts[1].trim();
      }
  ...
  const handleSaveEdit = () => {
    ...
    if (confirmed && editingData) {
      const newAngle = (editMinAngle && editMaxAngle) ? `${editMinAngle}° - ${editMaxAngle}°` : '-';

      setRoutine(prev => {
        const dayRoutine = prev[editingData.day];
        const updatedRoutine = dayRoutine.map(ex => {
          if (ex.id === editingData.exId) {
            return {
              ...ex,
              series: Number(editSets) || ex.series,
              reps: Number(editReps) || ex.reps,
              angle: newAngle
            };
          }
          return ex;
        ...
        return {
          ...prev,
          [editingData.day]: updatedRoutine
        };
      });
  ...
  if (!patient) {
    return (
      <SafeAreaView style={s.centerView}>
        <Text style={s.emptyViewText}>Paciente no encontrado</Text>
      </SafeAreaView>
    );
  }
                  ...
                  <View nativeID={day === 'Lunes' ? 'tutorial-assign-btn' : undefined}>
                    <TouchableOpacity 
                      ...
                      onPress={() => router.push({
                        ...
                        params: { patientId: patient.id, day: day }
                      })}
                    >
                    ...
                    {dayExercises.length === 0 ? (
                      ...
                      dayExercises.map((exercise) => (
                        <CompactRoutineCard 
                          ...
                          onEdit={() => handleOpenEdit(day, exercise)}       // <-- Le pasamos la funcion para editar
                          onDelete={() => handleDeleteExercise(day, exercise.id)} // <-- Le pasamos la funcion para borrar
                        />
                      ))
                    )}
                  </View>
